...
float Matrix::Discrete_Convolution(const Matrix &I, const Matrix &K, size_t i, size_t j, Error *err) const
{
    // Input validation
    if (i + K.row_count() > I.row_count() || j + K.col_count() > I.col_count())
    {
        if (err)
            *err = DIMENSION_MISMATCH;
        return 0.0f;
    ...
    for (size_t m = 0; m < K.row_count(); m++)
    {
        for (size_t n = 0; n < K.col_count(); n++)
        {
            // Direct accumulation - no intermediate val variable needed
            result += I(i + m, j + n, err) * K(m, n, err);

            // Early return if error occurred in element access
            if (err && *err != NO_ERROR)
            {
                return 0.0f;
            }
 ...
 Matrix Matrix::Convolve2D(const Matrix &input, const Matrix &kernel,
                          size_t stride, Error *err) 
{
    // Calculate output dimensions correctly with stride
    // Formula: output_size = (input_size - kernel_size) / stride + 1
    if (input.row_count() < kernel.row_count() || input.col_count() < kernel.col_count()) {
        if (err) *err = DIMENSION_MISMATCH;
        return Matrix(); // Return empty matrix
    }
    
    size_t Ho = (input.row_count() - kernel.row_count()) / stride + 1;
    size_t Wo = (input.col_count() - kernel.col_count()) / stride + 1;
    ...
    for (size_t i = 0; i < Ho; i++) {
        for (size_t j = 0; j < Wo; j++) {  // Fixed: Wo instead of Ho
            // Apply stride to input coordinates
            size_t input_i = i * stride;
            size_t input_j = j * stride;
            
            output(i, j) = output.Discrete_Convolution(input, kernel, input_i, input_j, err);
            
            // Check for errors after convolution
            if (err && *err != NO_ERROR) {
                return Matrix();
            }
        }
